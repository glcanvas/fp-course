...
removeByIndex pos array
      ...
      | otherwise = getItem 0 array
                where
                    ...
                    getItem n (x:xs) = if n < pos then getItem (n + 1) xs else Just x
                    getItem _ [] = Nothing
                                  ...
                                  let first = mergeSort $ firstPath array middle in
                                      let second = mergeSort $ secondPath array middle 0 in
                                          ...
                                          where
                                              ...
                                              secondPath [] _ _ = []
                                              secondPath (x:xs) middle n1 = if n1 < middle then
                                                                              secondPath xs middle (n1 + 1)
                                                                           else
                                                                              x : secondPath xs middle (n1 + 1)

                                              mergeArray :: [a] -> [a] -> [a]
                                              mergeArray [] ys = ys
                                              mergeArray xs [] = xs
                                              mergeArray (x:xs) (y:ys) = if x < y then
                                                                            x : mergeArray xs (y:ys)
                                                                         else
                                                                            y : mergeArray (x:xs) ys