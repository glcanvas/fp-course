...
-- task2
newtype Lord = Lord String deriving Show
newtype Castle = Castle (Maybe Lord) deriving Show
data Wall = Wall deriving Show
data Defence = Awful | Weak Castle | Strong Castle Wall deriving Show
data Civil = Library | Church | Wild  deriving Show
...
data Houses = HousesOne House | HousesMore House Houses deriving Show
data City = City {defence :: Defence, civil :: Civil, houses :: Houses}
  deriving Show
...
deleteElemTree elemTree (Branch array leftBranch rightBranch) =
  let firstElem = head array
   in case compare elemTree firstElem of
        EQ ->
          if null (tail array)
